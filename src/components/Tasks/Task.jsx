...
const Task = ({id, text, completed, onRemoveTask, onEditText}) => {
    const [isComplited, compliteTask] = useState(completed)

    const completeTask = () => {
        compliteTask(!isComplited);
        ...
        axios.patch(`http://localhost:3001/tasks/${id}`,
                ...
                .catch((err) => {
                    alert('Не удалось изменить состояние задачи!');
                    console.log(err);
                });
    }
    ...
    return(
        <div key={id} className="tasks__items-row">
            <input id={`task-${id}`} type="checkbox" checked={isComplited}/>
            <label htmlFor={`task-${id}`} onClick={completeTask}>
                <svg
                    width="11"
                    height="8"
                    viewBox="0 0 11 8"
            ...
            <div className="tasks__items-row-actions">
                <div onClick={() => onEditText(id, text)}>
                    <svg
                        width="15"
                        height="15"
                        viewBox="0 0 15 15"
